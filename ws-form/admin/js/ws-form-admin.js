(function($) {
	...
	$.WS_Form.prototype.settings_dual_list_init = function(obj) {
		...
		// Same paths as icon 'sort' (settings pages may not load settings_form.icons)
		var icon_drag = '<svg viewBox="0 0 16 16" width="16" height="16" aria-hidden="true"><path d="M11 7h-6l3-4z"/><path d="M5 9h6l-3 4z"/></svg>';
		...
		dual_list_selects.each(function() {
			...
			var label_available = ws_this.language('dual_list_available', false, false);
			var label_selected = ws_this.language('dual_list_selected', false, false);
			...
			var label_group_toggle = ws_this.language('dual_list_group_toggle', false, false);
			var label_item_toggle = ws_this.language('dual_list_item_toggle', false, false);
			var label_item_in_group = ws_this.language('dual_list_item_in_group', false, false);
			var label_available_esc = ws_this.esc_html(label_available);
			var label_selected_esc = ws_this.esc_html(label_selected);
			var label_available_id = select_id ? (select_id + '_dual_list_available_label') : '';
			var label_selected_id = select_id ? (select_id + '_dual_list_selected_label') : '';
			var count_available_id = select_id ? (select_id + '_dual_list_available_count') : '';
			var count_selected_id = select_id ? (select_id + '_dual_list_selected_count') : '';

			// Source select is mirrored by the dual-list UI
			select_obj.attr({
			...
			dual_list_html += '<div class="wsf-dual-list-heading"><span class="wsf-dual-list-heading-label"' + (label_available_id ? ' id="' + ws_this.esc_attr(label_available_id) + '"' : '') + '>' + label_available_esc + '</span><span class="wsf-dual-list-count wsf-dual-list-count-available"' + (count_available_id ? ' id="' + ws_this.esc_attr(count_available_id) + '"' : '') + ' aria-live="polite">0</span></div>';
			...
			dual_list_html += '<ul class="wsf-dual-list-items wsf-dual-list-available" role="listbox" aria-multiselectable="true"' + (label_available_id ? ' aria-labelledby="' + ws_this.esc_attr(label_available_id) + '"' : ' aria-label="' + ws_this.esc_attr(label_available) + '"') + (count_available_id ? ' aria-describedby="' + ws_this.esc_attr(count_available_id) + '"' : '') + ' tabindex="0"></ul>';
			dual_list_html += '</div>';
			...
			dual_list_html += '<div class="wsf-dual-list-heading"><span class="wsf-dual-list-heading-label"' + (label_selected_id ? ' id="' + ws_this.esc_attr(label_selected_id) + '"' : '') + '>' + label_selected_esc + '</span><span class="wsf-dual-list-count wsf-dual-list-count-selected"' + (count_selected_id ? ' id="' + ws_this.esc_attr(count_selected_id) + '"' : '') + ' aria-live="polite">0</span></div>';
			...
			dual_list_html += '<ul class="wsf-dual-list-items wsf-dual-list-selected" role="listbox" aria-multiselectable="true"' + (label_selected_id ? ' aria-labelledby="' + ws_this.esc_attr(label_selected_id) + '"' : ' aria-label="' + ws_this.esc_attr(label_selected) + '"') + (count_selected_id ? ' aria-describedby="' + ws_this.esc_attr(count_selected_id) + '"' : '') + ' tabindex="0"></ul>';
			dual_list_html += '</div>';
			...
			var min_selected = parseInt(select_obj.attr('data-wsf-dual-list-min'), 10);
			if(isNaN(min_selected) || (min_selected < 0)) { min_selected = 0; }

			// Export column picker only — Selected list is flat + sortable
			var reorder = select_obj.is('[data-wsf-dual-list-reorder]');
			var selected_order = [];
			var label_drag = reorder ? ws_this.language('dual_list_drag', false, false) : '';

			if(reorder) {

				var order_attr = select_obj.attr('data-wsf-dual-list-order') || '';

				if(order_attr !== '') {

					selected_order = order_attr.split(',').map(function(value) {
				...
				dual_list_obj.addClass('wsf-dual-list-reorder');
			}
			...
			var build_options_cache = function() {
				...
				for(var i = 0; i < options.length; i++) {
					...
					if(group_key && !group_seen[group_key]) {

						group_seen[group_key] = true;
					...
					var item = {
						...
						group_esc: ws_this.esc_html(group_label),
						group_attr: ws_this.esc_attr(group_key)
					};
			...
			var item_accessible_name = function(item) {

				if(!item.group_label) { return item.text; }

				return String(label_item_in_group)
					.replace('%1$s', item.text)
					.replace('%2$s', item.group_label);
			};

			var row_html = function(item, highlighted) {

				var aria_label = item_accessible_name(item);

				return '<li class="wsf-dual-list-item' + (highlighted ? ' wsf-dual-list-item-selected' : '') + '" role="option" data-value="' + item.value_esc + '" data-group="' + item.group_attr + '" title="' + ws_this.esc_attr(label_item_toggle) + '" aria-label="' + ws_this.esc_attr(aria_label) + '" aria-selected="' + (highlighted ? 'true' : 'false') + '" tabindex="-1">' + item.text_esc + '</li>';
			};

			var row_html_reorder = function(item, highlighted) {

				var aria_label = item_accessible_name(item);
				var html = '<li class="wsf-dual-list-item wsf-dual-list-item-reorder' + (highlighted ? ' wsf-dual-list-item-selected' : '') + '" role="option" data-value="' + item.value_esc + '" data-group="' + item.group_attr + '" title="' + ws_this.esc_attr(label_item_toggle) + '" aria-label="' + ws_this.esc_attr(aria_label) + '" aria-selected="' + (highlighted ? 'true' : 'false') + '" tabindex="-1">';
				html += '<span class="wsf-dual-list-item-handle" title="' + ws_this.esc_attr(label_drag) + '" aria-label="' + ws_this.esc_attr(label_drag) + '">' + icon_drag + '</span>';
				html += '<span class="wsf-dual-list-item-label" aria-hidden="true">' + item.text_esc + '</span>';

				if(item.group_esc) {

					html += '<span class="wsf-dual-list-item-group" aria-hidden="true">' + item.group_esc + '</span>';
				}
			...
			var group_html = function(group_key, group_label_esc, group_attr, group_label) {

				var group_aria = group_label + '. ' + label_group_toggle;

				return '<li class="wsf-dual-list-group" role="button" tabindex="0" data-group="' + group_attr + '" title="' + ws_this.esc_attr(label_group_toggle) + '" aria-label="' + ws_this.esc_attr(group_aria) + '">' + group_label_esc + '</li>';
			};

			var publish_selected_order = function() {

				if(!reorder) { return; }

				select_obj.data('wsfDualListSelectedOrder', selected_order.slice());
			};

			var sync_selected_order = function() {
				...
				for(i = 0; i < options_cache.length; i++) {

					if(options_cache[i].option.selected) {

						selected_set[options_cache[i].value_key] = true;
					}
				}

				var next_order = [];

				for(i = 0; i < selected_order.length; i++) {

					var order_key = String(selected_order[i]);

					if(selected_set[order_key]) {

						next_order.push(order_key);
						delete selected_set[order_key];
					}
				}

				for(i = 0; i < options_cache.length; i++) {

					if(selected_set[options_cache[i].value_key]) {

						next_order.push(options_cache[i].value_key);
					}
				}

				selected_order = next_order;
				publish_selected_order();
			};

			var sort_items_by_selected_order = function(items) {

				var order_index = {};

				for(var i = 0; i < selected_order.length; i++) {

					order_index[selected_order[i]] = i;
				}

				items.sort(function(a, b) {

					var a_index = (typeof order_index[a.value_key] === 'undefined') ? 999999 : order_index[a.value_key];
					var b_index = (typeof order_index[b.value_key] === 'undefined') ? 999999 : order_index[b.value_key];
			...
			var selected_count_total = function() {

				var count = 0;

				for(var i = 0; i < options_cache.length; i++) {

					if(options_cache[i].option.selected) { count++; }
				}
			...
			var update_buttons = function() {
				...
				btn_remove_all.disabled = (visible_selected_count === 0) || ((min_selected > 0) && (selected_count_total() <= min_selected));
			};
			...
			var render_grouped_parts = function(items, highlighted) {
				...
				for(i = 0; i < items.length; i++) {
					...
					if(item.group_key) {
						...
						if(typeof by_group[item.group_key] === 'undefined') {

							by_group[item.group_key] = [];
						}

						by_group[item.group_key].push(item);
				...
				sort_items_alpha(ungrouped);

				for(i = 0; i < ungrouped.length; i++) {

					parts.push(row_html(ungrouped[i], !!highlighted[ungrouped[i].value_key]));
				}

				for(i = 0; i < group_order.length; i++) {

					var group_key = group_order[i];
					var group_items = by_group[group_key];

					if(typeof group_items === 'undefined' || !group_items.length) { continue; }

					sort_items_alpha(group_items);
					parts.push(group_html(group_key, group_items[0].group_esc, group_items[0].group_attr, group_items[0].group_label));

					for(var g = 0; g < group_items.length; g++) {

						parts.push(row_html(group_items[g], !!highlighted[group_items[g].value_key]));
					}
				}
			...
			var refresh = function() {
				...
				var available_parts = render_grouped_parts(available_items, next_highlighted_available);
				var selected_parts;

				if(reorder) {

					sync_selected_order();
					sort_items_by_selected_order(selected_items);
					selected_parts = [];

					for(var s = 0; s < selected_items.length; s++) {

						selected_parts.push(row_html_reorder(selected_items[s], !!next_highlighted_selected[selected_items[s].value_key]));
					}

				} else {

					selected_parts = render_grouped_parts(selected_items, next_highlighted_selected);
				}

				visible_available_count = available_items.length;
				visible_selected_count = selected_items.length;
				...
				if(reorder && $(selected_el).hasClass('ui-sortable')) {

					$(selected_el).sortable('refresh');

					var can_sort = ((search_selected_el.value || '') === '') && (selected_items.length > 1);
					$(selected_el).sortable(can_sort ? 'enable' : 'disable');
				}
			};
			...
			var move_values = function(values, selected) {
				...
				// Enforce minimum selected count when deselecting
				if(!selected && (min_selected > 0)) {

					var remaining = selected_count_total();
					var filtered = [];

					for(var v = 0; v < values.length; v++) {

						var check_option = option_by_value[String(values[v])];

						if(!check_option || !check_option.selected || check_option.disabled) { continue; }

						if(remaining <= min_selected) { break; }
			...
			var visible_group_values = function(list_el, group_key) {
				...
				for(var i = 0; i < items.length; i++) {
					...
					if(String(items[i].getAttribute('data-group')) === String(group_key)) {
			...
			var handle_list_click = function(list_el, highlighted, set_anchor, get_anchor, clear_other, e) {
				...
				// Drag handle owns pointer interactions
				if(e.target.closest('.wsf-dual-list-item-handle')) { return; }
			...
			// Click optgroup header → highlight all visible items in that group
			var highlight_group = function(list_el, highlighted, set_anchor, clear_other, group_key) {

				var values = visible_group_values(list_el, group_key);
				if(!values.length) { return; }

				clear_other();

				for(var key in highlighted) { delete highlighted[key]; }

				for(var i = 0; i < values.length; i++) {

					highlighted[String(values[i])] = true;
				}
				...
				for(var j = 0; j < items.length; j++) {
					...
					if(String(items[j].getAttribute('data-group')) === String(group_key)) {

						last_index = j;
					}
				}

				set_anchor(last_index);
				paint_list_highlight(list_el, highlighted);
				update_buttons();
			};

			$(available_el).on('click', '.wsf-dual-list-group', function(e) {
			...
			$(selected_el).on('click', '.wsf-dual-list-group', function(e) {
			...
			$(available_el).on('keydown', '.wsf-dual-list-group', function(e) {
			...
			$(selected_el).on('keydown', '.wsf-dual-list-group', function(e) {
			...
			// Double-click optgroup header → move all visible items in that group
			$(available_el).on('dblclick', '.wsf-dual-list-group', function(e) {

				e.preventDefault();
				move_values(visible_group_values(available_el, $(this).attr('data-group')), true);
			});

			$(selected_el).on('dblclick', '.wsf-dual-list-group', function(e) {

				e.preventDefault();
				move_values(visible_group_values(selected_el, $(this).attr('data-group')), false);
			});
			...
			$(selected_el).on('dblclick', '.wsf-dual-list-item', function(e) {
				...
				if($(e.target).closest('.wsf-dual-list-item-handle').length) { return; }
			...
			if(reorder && (typeof $.fn.sortable === 'function')) {

				$(selected_el).sortable({

					axis: 'y',
					handle: '.wsf-dual-list-item-handle',
					items: '> .wsf-dual-list-item',
					tolerance: 'pointer',
					update: function() {

						if((search_selected_el.value || '') !== '') { return; }

						selected_order = visible_values(selected_el);
						publish_selected_order();
						select_obj.trigger('change');
					}
				});
	...
	$.WS_Form.prototype.form_render = function() {
		...
		// Groups - Tabs - Add
		var add_group_tab = '<li class="wsf-group-add wsf-ui-cancel" role="tab"><button' + this.esc_attr_tooltip(this.language('add_group'), 'top-center') + ' tabindex="-1">' + this.svg('group') + this.svg('plus') + '</button></li>';
	...
	$.WS_Form.prototype.group_render = function(obj) {
		...
		if(!obj.find('.wsf-group-header').length) {
			...
			var section_interface_html = '<div class="wsf-section-add" data-group-id="' + this.esc_attr(group_id) + '"><button' + this.esc_attr_tooltip(this.language('add_section'), 'left') + '>' + this.svg('section') + this.svg('plus') + '</button></div>';
	...
	$.WS_Form.prototype.field_render = function(obj, field) {
		...
		field_html += '<input type="text" data-wsf-value="' + this.esc_attr(this_field_label) + '" data-label="' + this.esc_attr(field_id) + '" aria-label="' + this.esc_attr(this.language('field_label')) + '" readonly maxlength="1024" autocomplete="off" data-private="lipsum" data-lpignore="true" data-1p-ignore></div>';
	...
	$.WS_Form.prototype.label_init = function(object_wrapper) {
		...
		// Set input value (This is done to prevent autofill software from interfering with the value)
		var label_obj = $('input[data-wsf-value]', object_wrapper).first();
		if(label_obj.length) {

			var label_value = label_obj.attr('data-wsf-value');
			label_obj.val(label_value).removeAttr('data-wsf-value');
		}

		// Get label object
		var label_obj = $('input', object_wrapper).first();
...
//		this.sidebar_reset();
		...
		for(var key in fieldsets) {
			...
			if(typeof fieldset.meta_keys !== 'undefined') {
				...
				for(var key in fieldset.meta_keys) {
					...
					// Field attributes - Standard (class_field: string or string[] from PHP JSON)
					var class_field = '';
					var cf = meta_key_config.class_field;
					if((typeof cf !== 'undefined') && (cf !== null)) {
						...
						if(Array.isArray(cf)) {
							...
							for(cf_i = 0; cf_i < cf.length; cf_i++) {

								if(typeof cf[cf_i] === 'string') {
							...
							if(cf_parts.length > 0) {

								class_field = ' ' + cf_parts.join(' ');
							}
					...
					if((typeof meta_key_config.compatibility_url !== 'undefined') && $.WS_Form.settings_plugin.helper_compatibility && !repeater) {
						...
						meta_key_options.push('<li><a class="wsf-compatibility" href="' + this.esc_url(meta_key_config.compatibility_url) + '" target="_blank"' + this.esc_attr_tooltip(this.language('attribute_compatibility'), 'top-right') + ' tabindex="-1">' + this.svg('markup') + '</a></li>');
					}
					...
					if(typeof meta_key_config.select_list === 'object') {
						...
						meta_key_options.push('<li><div data-action="wsf-select-list" data-option-meta-key="' + this.esc_attr(meta_key) + '"' + this.esc_attr_tooltip(this.language('select_list'), 'top-right') + '>' + this.svg('menu') + '</div></li>');
					...
					if((typeof meta_key_config.auto_map !== 'undefined') && !repeater) {
						...
						meta_key_options.push('<li><div data-action="wsf-auto-map" data-option-meta-key="' + this.esc_attr(meta_key) + '" data-object="' + this.esc_attr(object) + '" data-object-id="' + this.esc_attr(object_id) + '"' + this.esc_attr_tooltip(this.language('auto_map'), 'top-right') + '>' + this.svg('exchange') + '</div></li>');
					...
					if((typeof meta_key_config.reload !== 'undefined')) {
						...
						meta_key_options.push('<li><span data-action="wsf-api-reload"' + reload_attributes + ' data-meta-key-for="' + this.esc_attr(meta_key) + '"' + this.esc_attr_tooltip(this.language('action_api_reload'), 'top-right') + '>' + this.svg('reload') + '</span></li>');
					...
					switch(meta_key_type) {
						...
						case 'button_url' :
							...
							var button_url_href = (typeof meta_key_config.url !== 'undefined') ? meta_key_config.url : '';
							if(
								(object === 'form') &&
								(typeof this.form_id !== 'undefined') &&
								this.form_id
							) {
								button_url_href = this.mask_parse(button_url_href, { 'form_id' : String(parseInt(this.form_id, 10)) });
							}
	...
	$.WS_Form.prototype.sidebar_inits = function(inits, obj_sidebar_outer, obj_sidebar_inner, object_data) {
		...
		// Initialize number min/max/step clamp
		if(inits.indexOf('number') != -1) {
		...
		$('[data-change-event]', obj_inner).each(function() {
			...
			$(this).on(change_event, function() {
				...
				switch(change_action) {
					...
					// Rebuilds sidebar HTML; active tab resets
					case 'reload' :
					...
					// Persists without rebuilding sidebar (keeps active tab)
					case 'save' :

						$.WS_Form.this.object_save(obj);

						var object_type_save = $.WS_Form.this.get_object_type(obj);
						var obj_sidebar_outer_save = $('#wsf-sidebar-' + object_type_save);
						if(obj_sidebar_outer_save.length) {

							$.WS_Form.this.sidebar_condition_process(obj_sidebar_outer_save, obj_sidebar_outer_save, false);
						}
						break;
				}
			});
	...
	$.WS_Form.prototype.sidebar_knowledgebase = function(obj_sidebar_outer, obj_sidebar_inner) {
		...
		// Sidebar buttons use type=button, so handle click rather than form submit
		$('#wsf_contact_submit_lite').on('click', function(e) {
	...
	$.WS_Form.prototype.sidebar_repeater_html = function(repeater) {
		...
		// Add button
		repeater_html += '<div data-action="wsf-repeater-row-add"><div' + this.esc_attr_tooltip(this.language('repeater_row_add'), 'left') + '>' + this.svg('plus-circle') + '</div></div>';
	...
	$.WS_Form.prototype.sidebar_repeater_row_html = function(repeater, repeater_data_row, repeater_meta_key) {
		...
		// Sort
		repeater_row_html += '<td data-icon><div data-action="wsf-repeater-row-sort"' + this.esc_attr_tooltip(this.language('repeater_row_sort'), 'top-left') + '>' + this.svg('sort') + '</div></td>'
		...
		repeater_row_html += '<td data-icon><div data-action="wsf-repeater-row-delete"' + this.esc_attr_tooltip(this.language('repeater_row_delete'), 'top-right') + '>' + this.svg('delete-circle') + '</div></td>';
	...
	$.WS_Form.prototype.sidebar_number_init = function(obj_inner) {
		...
		$('input[type="number"][data-meta-key]', obj_inner).each(function() {

			var obj = $(this);

			// Clamp saved invalid values when sidebar opens
			$.WS_Form.this.sidebar_number_clamp(obj);

			obj.off('blur.wsf_number').on('blur.wsf_number', function() {

				$.WS_Form.this.sidebar_number_clamp($(this));
			});
		});
	}

	// Sidebar - Number - Clamp to min/max/step
	$.WS_Form.prototype.sidebar_number_clamp = function(obj) {
		...
		var min_attr = obj.attr('min');
		var max_attr = obj.attr('max');
		var step_attr = obj.attr('step');
		var has_min = (typeof min_attr !== 'undefined') && (min_attr !== '') && !isNaN(parseFloat(min_attr));
		var has_max = (typeof max_attr !== 'undefined') && (max_attr !== '') && !isNaN(parseFloat(max_attr));
		var min = has_min ? parseFloat(min_attr) : null;
		var max = has_max ? parseFloat(max_attr) : null;
		var num = parseFloat(value);

		// Non-numeric
		if(isNaN(num)) {

			var fallback = has_min ? String(min) : '';
			if(value !== fallback) {

				obj.val(fallback).trigger('change');
			}
			return;
		}
		...
		// Snap to step
		if(
			(typeof step_attr !== 'undefined') &&
			(step_attr !== '') &&
			(step_attr !== 'any')
		) {

			var step = parseFloat(step_attr);

			if(!isNaN(step) && (step > 0)) {

				var base = has_min ? min : 0;
				num = base + (Math.round((num - base) / step) * step);

				// Avoid float noise
				var step_decimals = ((String(step).split('.')[1]) || '').length;
				num = parseFloat(num.toFixed(step_decimals));
			}
		}

		// Min / max
		if(has_min && (num < min)) { num = min; }
		if(has_max && (num > max)) { num = max; }

		// Prefer integer string when whole number
		var clamped = (num === Math.floor(num)) ? String(Math.floor(num)) : String(num);

		if(value !== clamped) {

			obj.val(clamped).trigger('change');
		}
	}
		...
		for(var template_category_index in $.WS_Form.templates_section) {
			...
			for(var template_index in templates) {
				...
				if(template_category_download) {
					...
					section_select_html_templates += '<div data-action="wsf-section-download"' + this.esc_attr_tooltip(this.language('section_download'), 'top-right') + '>' + this.svg('download') + '</div>';
				}

				// Delete icon
				if(template_category_delete) {

					section_select_html_templates += '<div data-action="wsf-section-delete"' + this.esc_attr_tooltip(this.language('section_delete'), 'top-right') + '>' + this.svg('delete') + '</div>';
				}
			...
			if(section_select_html_templates != '') {
				...
				if(template_category_upload) {
					...
					li_array.push('<li><div data-action="wsf-section-upload"' + this.esc_attr_tooltip(this.language('section_selector_import'), 'top-right') + '>' + this.svg('upload') + '</div><input type="file" class="wsf-section-upload" id="wsf-section-upload-file" accept=".json"/></li>');
				}
	...
	$.WS_Form.prototype.data_grid_html = function(meta_key, meta_value, data_source_id, read_only, bulk_action_row_ids) {
		...
		if(upload_download) {
			...
			if(!read_only) {
				...
				li_array.push('<li><div data-action="wsf-data-grid-upload"' + this.esc_attr_tooltip(this.language('data_grid_group_upload_csv'), 'bottom-right') + '>' + this.svg('upload') + '</div><input type="file" class="wsf-file-upload" id="wsf-data-grid-upload-file" accept=".csv"/></li>');
			}

			// Download CSV
			li_array.push('<li><div data-action="wsf-data-grid-download"' + this.esc_attr_tooltip(this.language('data_grid_group_download_csv'), 'bottom-right') + '>' + this.svg('download') + '</div></li>');
		}
	...
	$.WS_Form.prototype.data_grid_html_group = function(group, group_index, columns, rows_per_page, meta_key, read_only, bulk_action_row_ids) {
		...
		if(!read_only) {
			...
			return_html += '<div data-action="wsf-data-grid-row-add"><div' + this.esc_attr_tooltip(this.language('data_grid_row_add'), 'left') + '>' + this.svg('plus-circle') + '</div></div>';
		}
		...
		if(!read_only && group_settings_show) {
			...
			// Group - Settings - Open
			return_html += '<li class="wsf-flex-none"><div data-action="wsf-data-grid-settings"' + this.esc_attr_tooltip(this.language('data_grid_settings'), 'top-right') + '>' + this.svg('settings') + '</div></li>';
		}
	...
	$.WS_Form.prototype.data_grid_html_row_header = function(columns, group_index, row_default, row_disabled, row_required, row_hidden, meta_key, read_only) {
		...
		if(!read_only) {
			...
			return_html += '<th data-fixed-select class="wsf-data-grid-checkbox"' + this.esc_attr_tooltip(this.language('data_grid_row_bulk_select_all'), 'right') + '><input id="' + this.esc_attr(row_id) + '" class="wsf-field" data-action="wsf-data-grid-row-select-all" type="checkbox" /><label for="' + this.esc_attr(row_id) + '" class="wsf-label"></label></th>'
		}
		...
		for(var key in columns) {
			...
			if(!read_only && (columns.length > 1) && ((max_columns > 1) || (max_columns == 0))) {
				...
				return_html += '<div data-action="wsf-data-grid-column-delete" data-id="' + this.esc_attr(column_id) + '"' + this.esc_attr_tooltip(this.language('data_grid_column_delete'), 'bottom-center') + '>' + this.svg('delete-circle') + '</div>';
			}
		...
		if(
			...
			// Spacer - Add column
			return_html += '<th data-fixed-icon><div data-action="wsf-data-grid-column-add"' + this.esc_attr_tooltip(this.language('data_grid_column_add'), 'top-right') + '>' + this.svg('plus-circle') + '</div></th>';
		}
	...
	$.WS_Form.prototype.data_grid_html_row = function(row, group_index, row_index, row_default, row_disabled, row_required, row_hidden, meta_key, read_only, bulk_action_row_ids) {
		...
		if(!read_only) {
			...
			// Sort
			return_html += '<td data-fixed-sort><div data-action="wsf-data-grid-row-sort"' + this.esc_attr_tooltip(this.language('data_grid_row_sort'), 'top-left') + '>' + this.svg('sort') + '</div></td>'

			// Selector
			var bulk_action_row_checked = ((typeof bulk_action_row_ids === 'object') && (bulk_action_row_ids.indexOf(row_index) !== -1));
			return_html += '<td data-fixed-select' + this.esc_attr_tooltip(this.language('data_grid_row_bulk_select'), 'right') + '><input data-action="wsf-data-grid-row-select" id="' + this.esc_attr(row_id) + '" class="wsf-field" type="checkbox" tabindex="-1"' + (bulk_action_row_checked ? ' checked' : '') + ' /><label for="' + this.esc_attr(row_id) + '" class="wsf-label"></label></td>'
		}
		...
		for(var column_index = 0; column_index < column_count; column_index++) {
			...
			if(insert_image) {
				...
				return_html += '<div data-action="wsf-insert-image"' + this.esc_attr_tooltip(this.language('data_grid_insert_image'), 'left') + '>' + this.svg('file-picture') + '</div>';
			}
		...
		// Supported attributes
		if(row_default) { return_html += '<td data-fixed-icon><div data-attribute="default" data-status="' + (row.default ? 'on': '') + '"' + (read_only ? ' data-read-only' : '') + this.esc_attr_tooltip(this.language('data_grid_row_default'), 'top-center') + '>' + this.svg('check') + '</div></td>'; }
		if(row_required) { return_html += '<td data-fixed-icon><div data-attribute="required" data-status="' + (row.required ? 'on': '') + '"' + (read_only ? ' data-read-only' : '') + this.esc_attr_tooltip(this.language('data_grid_row_required'), 'top-center') + '>' + this.svg('asterisk') + '</div></td>'; }
		if(row_disabled) { return_html += '<td data-fixed-icon><div data-attribute="disabled" data-status="' + (row.disabled ? 'on': '') + '"' + (read_only ? ' data-read-only' : '') + this.esc_attr_tooltip(this.language('data_grid_row_disabled'), 'top-center') + '>' + this.svg('disabled') + '</div></td>'; }
		if(row_hidden) { return_html += '<td data-fixed-icon><div data-attribute="hidden" data-status="' + (row.hidden ? 'on': '') + '"' + (read_only ? ' data-read-only' : '') + this.esc_attr_tooltip(this.language('data_grid_row_hidden'), 'top-center') + '>' + this.svg('visible') + this.svg('hidden') + '</div></td>'; }

		// Delete
		if(!read_only) {

			return_html += '<td data-fixed-icon><div data-action="wsf-data-grid-row-delete"' + this.esc_attr_tooltip(this.language('data_grid_row_delete'), 'top-right') + '>' + this.svg('delete-circle') + '</div></td>';
		}
		...
		element.render = function(read_only, row_index_focus, bulk_action_row_ids) {
			...
			if(!read_only) {
				...
				$('[data-action="wsf-data-grid-auto-group"]', obj).on('change', function() {
					...
					// Ids for split groups: keep source group's id on first bucket, then monotonic new ids (sort must not change ids)
					var id_next = $.WS_Form.this.data_grid_group_id_next(meta_value);
					var first_split_group = true;
					...
					for(var row_index in rows) {
						...
						if(typeof groups_new[group_label] === 'undefined') {
							...
							if(first_split_group) {

								first_split_group = false;
								var id_src = parseInt(group.id, 10);
								groups_new[group_label].id = (!isNaN(id_src)) ? id_src : 0;
							} else {

								groups_new[group_label].id = id_next;
								id_next++;
							}
						}
				...
				if(!read_only) {
					...
					$('[data-action="wsf-data-grid-group-add"]', obj).on('click', function() {
						...
						if(typeof meta_value.groups[0] !== 'undefined') {
							...
							group_new.id = $.WS_Form.this.data_grid_group_id_next(meta_value);
	...
	// Data grid - Next group id (max existing + 1) for new groups
	$.WS_Form.prototype.data_grid_group_id_next = function(meta_value) {

		var id_max = -1;
		if(
			(typeof meta_value === 'undefined') ||
			(typeof meta_value.groups === 'undefined')
		) { return 0; }
		...
		for(var i = 0; i < groups.length; i++) {
			...
			if(
				...
				var n = parseInt(g.id, 10);
				if(!isNaN(n) && (n > id_max)) { id_max = n; }
			}
		}
	...
	$.WS_Form.prototype.confirm_modal = function(message, confirm_callback, options) {
		...
		if(!modal_obj.length) {
			...
			modal_html += '<div class="wsf-modal-content"></div>';
		...
		$('.wsf-modal-content', modal_obj).html(content_html);
	...
	$.WS_Form.prototype.settings_html = function(object, object_id, multiple, no_resize) {
		...
		for(var key in buttons) {
			...
			settings_html += '<li data-id="' + this.esc_attr(object_id) + '" data-action="' + this.esc_attr(button_method) + '"' + this.esc_attr_tooltip(button_name, 'top-center') + '>' + button_icon + '</li>';
		}
	...
	$.WS_Form.prototype.wp_list_table_submit_init = function() {
		...
		$('[data-action]:not([data-action="wsf-clipboard"])').on('click', function() {
			...
			// Export column picker / dual-list controls its own handlers
			if(
				action &&
				(
					(action.indexOf('wsf-submit-export-columns-') === 0) ||
					(action.indexOf('wsf-dual-list-') === 0)
				)
			) {
				return;
			}
		...
		// Export columns dual-list
		this.settings_dual_list_init($('#wsf-submit-export-columns-modal'));

		// Clear export column error when selection changes
		$('#wsf-submit-export-columns').off('change.wsfSubmitExportColumns').on('change.wsfSubmitExportColumns', function() {

			$.WS_Form.this.submit_export_columns_error_hide();
		});

		// Export columns modal
		$(document).off('click.wsfSubmitExportColumns').on('click.wsfSubmitExportColumns', '#wsf-submit-export-columns-modal-backdrop, [data-action="wsf-submit-export-columns-close"]', function(e) {

			e.preventDefault();
			e.stopPropagation();
			$.WS_Form.this.submit_export_columns_modal_hide();
		});

		$(document).off('click.wsfSubmitExportColumnsExport').on('click.wsfSubmitExportColumnsExport', '[data-action="wsf-submit-export-columns-export"]', function(e) {

			e.preventDefault();
			e.stopPropagation();
			$.WS_Form.this.submit_export_columns_start();
		});
	}

	// Submit export - open column picker
	$.WS_Form.prototype.submit_export = function(submit_ids) {

		// Store pending export filters
		this.submit_export_pending = {

			submit_ids: (typeof submit_ids !== 'undefined') ? submit_ids : false
		};

		if(!$('#wsf-submit-export-columns').length) {

			this.error('error_submit_export_columns');
			return;
		}

		this.submit_export_columns_modal_show();
	}

	// Save selection and start export
	$.WS_Form.prototype.submit_export_columns_start = function() {

		var select_el = document.getElementById('wsf-submit-export-columns');
		if(!select_el) {
		...
		var select_obj = $(select_el);
		var columns = select_obj.data('wsfDualListSelectedOrder');
		var columns_hidden = [];
		var options = select_el.options;
		var selected_set = {};
		var i;
		...
		for(i = 0; i < columns.length; i++) {
			...
			selected_set[String(columns[i])] = true;
		}

		for(i = 0; i < options.length; i++) {

			var value_key = String(options[i].value);

			if(!selected_set[value_key]) {

				columns_hidden.push(value_key);
			}
		}

		if(!columns.length) {

			this.submit_export_columns_error_show();
			return;
		}

		this.submit_export_columns_error_hide();

		var pending = this.submit_export_pending || {};
		var ws_this = this;

		// Progress UI immediately; save prefs, then start CSV
		this.submit_export_columns_modal_hide();
		this.submit_export_popup_show();

		this.api_call('submit/export/columns/', 'POST', {
			...
			ws_this.submit_export_run(pending.submit_ids, columns);

		}, function() {

			ws_this.submit_export_popup_hide();
			ws_this.submit_export_columns_modal_show();

		}, false, true, true);
	}

	// Build export request and run paged export
	$.WS_Form.prototype.submit_export_run = function(submit_ids, columns) {
		...
		// Current status tab (needed so spam/trash exports are not excluded)
		var status = this.submit_export_get_status();
		if(status) {

			data['ws-form-status'] = status;
		}
	...
	// Current submissions status from URL
	$.WS_Form.prototype.submit_export_get_status = function() {

		var match = window.location.search.match(/[?&]ws-form-status=([^&]*)/);
	...
	// Submit export columns modal - Show
	$.WS_Form.prototype.submit_export_columns_modal_show = function() {

		this.submit_export_columns_error_hide();
		$('#wsf-submit-export-columns-modal-backdrop').show();
		$('#wsf-submit-export-columns-modal').show();
	}

	// Submit export columns modal - Hide
	$.WS_Form.prototype.submit_export_columns_modal_hide = function() {

		this.submit_export_columns_error_hide();
		$('#wsf-submit-export-columns-modal').hide();
		$('#wsf-submit-export-columns-modal-backdrop').hide();
	}

	// Submit export columns modal - Error show
	$.WS_Form.prototype.submit_export_columns_error_show = function() {

		var error_obj = $('#wsf-submit-export-columns-error');
		if(!error_obj.length) { return; }

		$('p', error_obj).text(this.language('error_submit_export_columns', false, false));
		error_obj.show();
	}

	// Submit export columns modal - Error hide
	$.WS_Form.prototype.submit_export_columns_error_hide = function() {

		$('#wsf-submit-export-columns-error').hide();
	}
	...
	$.WS_Form.prototype.submit_render_do = function(submit, form, view) {
		...
		// Encrypted?
		var encrypted_html = (submit.encrypted) ? '<div class="wsf-encrypted"' + this.esc_attr_tooltip(this.language('submit_encrypted'), 'top-center') + '>' + this.svg('readonly') + '</div>' : '';
	...
	$.WS_Form.prototype.sidebar_render_actions = function(submit) {
		...
		for(var submit_action_index in submit_actions) {
			...
			if(sidebar_render_action_logs_html !== false) {
				...
				sidebar_actions_html += '<div data-toggle="wsf-action-logs-' + this.esc_attr(submit_action_index) + '"' + this.esc_attr_tooltip(this.language('submit_actions_logs'), 'top-center') + '>' + this.svg('info-circle') + '</div>';
			}
			...
			if(sidebar_render_action_errors_html !== false) {
				...
				sidebar_actions_html += '<div data-toggle="wsf-action-errors-' + this.esc_attr(submit_action_index) + '"' + this.esc_attr_tooltip(this.language('submit_actions_errors'), 'top-right') + '>' + this.svg('warning') + '</div>';
			}
			...
			if(action_can_repost) {
				...
				sidebar_actions_html += '<div data-action="wsf-submit-action-repost" data-submit-action-index="' + this.esc_attr(submit_action_index) + '"' + this.esc_attr_tooltip(this.language('submit_actions_repost'), 'top-right') + '>' + this.svg('redo') + '</div>';
			}
	...
	// Tooltip
	$.WS_Form.prototype.esc_attr_tooltip = function(title, position) {
	...
	$.WS_Form.prototype.variable_helper_modal_var_html = function(variable) {
		...
		if(variable.kb_slug) {
			...
			// Add icon
			tab_content_html += '<a class="wsf-vh-var-kb-url" href="' + this.esc_url(kb_url) + '" target="_blank"' + this.esc_attr_tooltip(this.language('field_kb_url'), 'left') + ' tabindex="-1">' + this.svg('question-circle') + '</a>';
		}
		...
		if(variable.kb_category_slug) {
			...
			// Add icon
			tab_content_html += '<a class="wsf-vh-var-kb-url" href="' + this.esc_url(kb_category_url) + '" target="_blank"' + this.esc_attr_tooltip(this.language('field_kb_url'), 'left') + ' tabindex="-1">' + this.svg('question-circle') + '</a>';
		}
		...
		// Variable syntax
		tab_content_html += '<div class="wsf-vh-var-code"><code' + this.esc_attr_tooltip(this.language('variable_insert'), 'right') + '>' + this.esc_html(variable.value) + '</code> <div class="wsf-vh-var-copy"' + this.esc_attr_tooltip(this.language('clipboard'), 'right') + '>' + this.svg('clone') + '</div></div>';
